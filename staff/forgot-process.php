...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['email'])) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $sql = "SELECT ID, Email FROM tblstaff WHERE Email = :email LIMIT 1";
                ...
                <div class="row flex-grow">
                    <div class="col-lg-4 mx-auto">
                        <div class="auth-form-light text-left p-5">
                            <div class="brand-logo" align="center" style="font-weight:bold">Staff Password Reset</div>
                            <h6 class="font-weight-light">Enter your staff email to receive a 6-digit reset code.</h6>
                            <?php if (!empty($error)): ?>
                                <div class="alert alert-danger"><?php echo htmlentities($error); ?></div><?php endif; ?>
                            <form class="pt-3" method="post">
                                <div class="form-group"><input type="email" class="form-control form-control-lg"
                                        name="email" placeholder="Email address" required autofocus></div>
                                <div class="mt-3"><button class="btn btn-success btn-block loginbtn" type="submit">Send
                                        OTP</button></div>
