...
function send_otp_email($toEmail, $code, $userName = 'Admin')
{
  global $hasPHPMailer, $MAIL_HOST, $MAIL_USERNAME, $MAIL_PASSWORD, $MAIL_PORT, $MAIL_ENCRYPTION, $MAIL_FROM, $MAIL_FROM_NAME;

  $currentYear = date('Y');
  $subject = 'Your Student Profiling System Admin Password Reset Code';
...
<div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
    <div style="background-color: #f4f4f4; padding: 20px; text-align: center; border-bottom: 1px solid #ddd;">
        <h2 style="color: #333; margin: 0;">Admin Password Reset Request</h2>
    </div>
    <div style="padding: 30px;">
        <p>Hi {$userName},</p>
        <p>We recently received a request to reset the password for your Student Profiling System admin account. To complete this process, please use the following One-Time Password (OTP) code:</p>
...
We recently received a request to reset the password for your Student Profiling System admin account. To complete this process, please use the following One-Time Password (OTP) code:
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['email'])) {
  ...
  if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    ...
    $sql = "SELECT ID, Email, AdminName FROM tbladmin WHERE Email = :email LIMIT 1";
    ...
    if ($stmt->rowCount() === 0) {
      ...
      $sent = send_otp_email($email, $code, $admin->AdminName);
      if ($sent) {
  ...
  <div class="container">
    <div class="welcome-section">
      <div class="welcome-content">
        ...
        <p>If you've forgotten your password, enter your email address below. We'll send you a secure code to reset it.
        </p>
      </div>
